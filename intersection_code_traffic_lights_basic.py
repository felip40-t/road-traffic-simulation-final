...
def write_flows(time):
    ...
    for rho in (densities):
        ...
        with open("flow_rate_intersection_3.csv", 'a', encoding='utf-8') as data:
            writer = csv.writer(data)
            writer.writerow([avg_density, flow_rate])

def plot_flows():
    data = np.genfromtxt("flow_rate_intersection_3.csv", dtype='float',
    ...
    ax.set_xlabel('Initial density (seconds)', fontsize=18)
    ...
    plt.show()
    plt.savefig("Flow_graph_intersection_int_6_1.pdf", dpi=400)
