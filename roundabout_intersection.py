...
def iterate_roads(total_time, density):
    ...
    while time < total_time:
        ...
        time += 1
    return flow / (time*5), densit / (time*5)
...
def write_flows(time):
    ...
    for rho in (densities):
        flow_rate, avg_density = iterate_roads(time, rho)
...
def plot_flows():
    ...
    ax.set_title('Traffic Flow for roundabout intersection', fontsize=22)
    ax.set_xlabel('Average density (cars per site)', fontsize=18)
    ax.set_ylabel('Average Flow rate (cars per time step)', fontsize=18)
    ...
    plt.show()
    plt.savefig("Flow_graph_intersection_round1.pdf", dpi=400)
