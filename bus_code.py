...
def influx_gen(road, vehicle_locs, locs_car, locs_bus):
...
def iterate_road(tot_time, rho_car, rho_bus):
    ...
    while t < tot_time:
        ...
        if influx:
            road, car_indices, bus_indices, vehicle_locs = influx_gen(road, vehicle_locs, car_indices, bus_indices)
        t += 1
    return flow / t

def write_flows(time):
    density_car = rand.uniform(0,1,200)
    density_bus = rand.uniform(0, (1 - density_car)/2 , 200)
    densities = []
    for i in range(200):
    ...
    with open("flow_rate_data_bus2.csv", 'a', encoding='utf-8') as data:
        writer = csv.writer(data)
        for index in enumerate(densities):
            writer.writerow([densities[index[0]][0],densities[index[0]][1], flow_rates[index[0]]])


def plot_flows():
    data = np.genfromtxt("flow_rate_data_bus2.csv", dtype='float',
    ...
    ax.set_title('Traffic Flow plot \n (L = 500 metres, Total time = 1500 seconds)', fontsize=12)
    ax.set_xlabel('Initial density of cars', fontsize=8)
    ax.set_ylabel('Initial density of busses', fontsize=8)
    ax.set_zlabel('Flow of traffic', fontsize=8)
    ax.scatter3D(density_car, density_bus, flow_rates, color='black', s=1)
    #plt.grid()
    plt.show()
    plt.savefig("Flow_graph_busses2.pdf", dpi=400)
