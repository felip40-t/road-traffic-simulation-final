...
road_length = 25  # metres / number of sites
...
def iterate_road(tot_time, rho_car, rho_bus):
    ...
    while t < tot_time:
        ...
        road, car_indices, bus_indices, vehicle_locs = moving_cars(
            road, car_indices, bus_indices)
        flow += car_flow(road, vehicle_locs)
        if influx:
            road, car_indices, bus_indices, vehicle_locs = influx_gen(
                road, vehicle_locs, car_indices, bus_indices, rho_car, rho_bus)
        t += 1
    return flow / t
...
# write_flows(seconds)
# plot_flows()
iterate_road(seconds, 0.25, 0.15)
end = time.time()
print((end - start)/60)
